...
console.log('🎯 FINAL Course Restoration - Complete with ALL Required Fields');
console.log('This will create fully functional, editable courses that work in both Studio AND frontend\n');

// Helper function to create properly structured blocks
...
// Complete courses with ALL required fields
const courses = [
  {
    ...
    description: 'Transform yourself from a complete beginner to a job-ready web developer.',
...

  {
    ...
    description: 'Learn to build native mobile applications for both iOS and Android using React Native.',
...

  {
    ...
    description: 'Master project management methodologies, tools, and techniques.',
...

  {
    ...
    description: 'Learn how to validate business ideas quickly and build minimum viable products.',
...

  {
    ...
    description: 'Complete digital marketing strategy for small businesses.',
    ...
    featured: false,
    prerequisites: ['Basic computer skills', 'Small business or entrepreneurial interest'],
    maxStudents: 45,
    curriculum: {
      modules: 8,
      lessons: 32,
...

  {
    ...
    description: 'Master advanced SEO techniques to dominate search results.',
    ...
    featured: true,
    prerequisites: ['Basic SEO knowledge', 'Website management experience'],
    maxStudents: 30,
    curriculum: {
      modules: 10,
      lessons: 40,
...

  {
    ...
    description: 'Learn data analysis and visualization using Python, pandas, NumPy, and matplotlib.',
...

  {
    ...
    description: 'Master Google Analytics 4 (GA4) with advanced tracking and custom reports.',
    ...
    featured: false,
    prerequisites: ['Basic web knowledge', 'Google account'],
    maxStudents: 50,
    curriculum: {
      modules: 6,
      lessons: 24,
...

  {
    ...
    featured: true,
    prerequisites: ['5+ years security experience', 'Bachelor\'s degree or equivalent'],
    maxStudents: 25,
    curriculum: {
      modules: 16,
      lessons: 64,
...

  {
    ...
    description: 'Learn ethical hacking fundamentals and penetration testing.',
...

  {
    ...
    description: 'Master Linux/Unix command line and shell scripting.',
...
async function deleteAllDocuments() {
  console.log('🗑️  Step 1: Complete cleanup of existing documents...\n');
...
async function createCategories() {
  console.log('📂 Step 2: Creating course categories...\n');
...
async function createCourses() {
  console.log('📚 Step 3: Creating complete, functional courses...\n');
  ...
  try {
    ...
    await transaction.commit();
    console.log('\n✅ All courses created with complete data!\n');
...
async function verifyEverything() {
  ...
  try {
    ...
    const courses = await client.fetch('*[_type == "course"]{title, slug, price, instructor, body}');
    ...
    courses.forEach(course => {
      const hasAllFields = !!(course.slug?.current && course.price && course.instructor);
      const hasEditableBody = course.body && course.body.length > 0 && course.body[0]._key;
      
      console.log(`   - ${course.title}`);
      console.log(`     Slug: ${course.slug?.current || 'MISSING'}`);
      console.log(`     Price: ${course.price || 'MISSING'}`);
      console.log(`     Instructor: ${course.instructor || 'MISSING'}`);
      console.log(`     Body: ${hasEditableBody ? '✅ Editable' : '❌ Not editable'}`);
      console.log(`     Status: ${hasAllFields && hasEditableBody ? '🎉 FULLY FUNCTIONAL' : '⚠️  Issues detected'}`);
      console.log('');
    });
    
    console.log('🎉 RESTORATION COMPLETE!');
    console.log('✅ All courses should now be:');
    console.log('   - Fully EDITABLE in Sanity Studio');
    console.log('   - Displaying correctly on frontend');
    console.log('   - Have all required fields');
    console.log('   - Include rich, detailed content');
