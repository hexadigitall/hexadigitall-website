...
// Usage: node scripts/download-courses-images.cjs
...
const schoolsStructure = require('./schools-structure.cjs');
...
async function downloadCourseImages() {
  ...
  for (const [school, schoolData] of Object.entries(schoolsStructure)) {
    for (const course of schoolData.courses) {
      const query = typeof course === 'string' ? course : course.title;
