...
export default async function BookPage({ params }: Props) {
  ...
  const structuredData = {
    ...
    author: book._type === 'imprint' ? { '@type': 'Person', name: book.author?.name } : book.authors?.map((a: string) => ({ '@type': 'Person', name: a })) ?? [],
          ...
          <div className="flex flex-col">
            <div className="mb-10">
              <div className="flex items-center gap-3 mb-6">
                <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full ${STATUS_STYLES[book.status]}`}>
                ...
                <span className="text-[10px] font-black uppercase tracking-widest bg-slate-950 text-white px-3 py-1.5 rounded-full border border-white/10">
                  {book._type === 'imprint' ? 'Digital Imprint' : 'Course Textbook'}
                </span>
              </div>
              
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold font-serif text-slate-950 dark:text-white leading-tight mb-4 tracking-tight">
                {book.title}
              </h1>
              ...
              <div className="flex items-center gap-4 py-4 border-y border-slate-100 dark:border-slate-800 mb-8">
                <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center text-white font-black text-xs shadow-lg shadow-blue-500/20">
            ...
            <div className="prose prose-slate dark:prose-invert max-w-none mb-12">
              <p className="text-lg leading-relaxed text-slate-600 dark:text-slate-400 font-serif whitespace-pre-line">
                {book.description}
              </p>
            </div>
            ...
            <Link href={`/courses/${book.relatedCourse.slug.current}`} className="group block bg-blue-600 rounded-3xl p-1 shadow-xl shadow-blue-500/20">
              <div className="bg-white dark:bg-slate-950 rounded-[22px] p-8 flex flex-col md:flex-row items-center justify-between gap-8 group-hover:bg-transparent transition-all duration-500">
                <div className="flex items-center gap-6">
                  <div className="w-16 h-16 bg-blue-50 dark:bg-blue-900/20 rounded-2xl flex items-center justify-center text-3xl group-hover:bg-white/20 transition-colors">
                    🎓
                  </div>
                  <div>
                ...
                <div className="text-blue-600 group-hover:text-white font-black uppercase tracking-widest text-xs flex items-center gap-2 transition-colors">
                  Explore Curriculum <ArrowRightIcon className="h-4 w-4" />
                </div>
              </div>
            </Link>
          </section>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {book.assets.map((asset: any) => (
                  <div key={asset._id} className="bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-6 hover:shadow-xl transition-all group">
                    ...
                    <button className="w-full py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-black uppercase tracking-widest text-slate-600 dark:text-slate-400 hover:bg-slate-950 hover:text-white transition-all">
                      Acquire Asset
                    </button>
                  </div>
                ))}
