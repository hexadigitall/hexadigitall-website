...
export async function POST(request: NextRequest) {
  try {
    ...
    const template: string = body.template || 'explorer'

    const defaultWidgetTypes = getDefaultWidgetsForTemplate(template)
    const widgets = defaultWidgetTypes.map((type, index) => ({
      widgetId: `${type}-${index}`,
      type,
      title: '',
      config: { items: [] },
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to reset homepage config' },
      { status: 500 }
    )
  }
}
