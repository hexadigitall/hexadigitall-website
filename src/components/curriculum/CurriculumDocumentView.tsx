...
export default function CurriculumDocumentView({ curriculum, showActions = true }: CurriculumDocumentViewProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white dark:bg-slate-900 dark:bg-slate-900 shadow-[0_24px_80px_-40px_rgba(15,23,42,0.35)]">
              ...
              {!!curriculum.heroTags?.length && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {curriculum.heroTags.map((tag, index) => (
                    <span key={`${tag}-${index}`} className="rounded-full border border-cyan-100/35 bg-white dark:bg-slate-900 dark:bg-slate-900/90 px-3 py-1 text-xs font-semibold text-cyan-800">
                      {tag}
                    </span>
                  ))}
                </div>
        ...
        <div className="space-y-8 px-6 py-8 sm:px-10 lg:px-12 lg:py-10">
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            ...
            {!!curriculum.essentialResources?.length && (
              <section className="rounded-[24px] border border-slate-200 bg-white dark:bg-slate-900 dark:bg-slate-900 p-6 sm:p-8">
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-4">
                {curriculum.weeks.map((week) => (
                  <article key={week._key || `${week.weekNumber}-${week.topic}`} className="rounded-[24px] border border-slate-200 bg-white dark:bg-slate-900 dark:bg-slate-900 p-6 sm:p-8">
            ...
            <section className="rounded-[24px] border border-slate-200 bg-slate-50 p-6 sm:p-8">
              ...
              <div className="mt-5 grid gap-4 xl:grid-cols-3">
                {curriculum.capstoneProjects.map((project, index) => (
                  <article key={`${project.title}-${index}`} className="rounded-2xl border border-slate-200 bg-white dark:bg-slate-900 dark:bg-slate-900 p-5">
