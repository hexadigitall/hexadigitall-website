...
export async function AvailableInStore() {
    ...
    <section className="py-16 md:py-20 bg-gradient-to-br from-sky-50 via-white to-emerald-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-900 border-y border-sky-100/60 dark:border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
          <div>
            <p className="text-xs font-semibold tracking-[0.18em] text-secondary dark:text-cyan-300 uppercase">
              Available Now
            </p>
            <h2 className="mt-2 text-2xl md:text-3xl font-bold text-darkText dark:text-slate-100">
              In Store Today
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {available.map((book) => {
            ...
            const typeLabel = TYPE_LABELS[book._type] || 'Publication'
            return (
              <article key={book._id} className="rounded-2xl border border-gray-100 bg-white shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                <Link href={`/store/${book.slug.current}`} className="block">
                  <div className="relative bg-gray-50" style={{ aspectRatio: '3/4' }}>
                    ...
                    <span className="absolute left-3 top-3 text-[11px] font-semibold px-2 py-1 rounded-full bg-white/90 text-emerald-700 border border-emerald-200/70">
                      {typeLabel}
                    </span>
                  </div>
                </Link>
                <div className="p-4">
                  ...
                  {(book.edition || book.authors?.length) && (
                    <p className="mt-1 text-xs text-gray-500">
                      {book.authors?.slice(0, 2).join(', ')}
                      {book.authors?.length ? (book.edition ? ` · ${book.edition}` : '') : (book.edition || '')}
                    </p>
                  )}
                </div>
              </article>
