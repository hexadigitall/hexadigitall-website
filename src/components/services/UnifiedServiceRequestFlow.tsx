...
export default function UnifiedServiceRequestFlow({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 overflow-y-hidden" onClick={onClose}>
      <div ref={dialogRef} tabIndex={-1} aria-modal="true" role="dialog" aria-label={`Service request flow for ${serviceName}`} className="bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-2xl max-w-2xl w-full p-6 md:p-8 relative my-8 max-h-[90vh] overflow-y-auto outline-none" onClick={(e) => e.stopPropagation()}>
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-gray-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-400 dark:text-slate-400 hover:text-gray-600 dark:text-slate-400 dark:text-slate-400 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center rounded-full hover:bg-gray-100"
          aria-label="Close"
        >
              ...
              return (
                <div key={step} className="flex flex-col items-center">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold transition-all ${
                    ...
                    isPast ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300' :
                    'bg-gray-200 text-gray-600 dark:text-slate-400'
                  }`}>
                    {isPast ? '✓' : idx + 1}
                  </div>
                  <p className="text-xs font-semibold text-gray-600 dark:text-slate-400 dark:text-slate-400 mt-2 capitalize">{step}</p>
                </div>
              )
            })}
          </div>
        ...
        {currentStep === 'review' && (
          <div>
            <h2 ref={stepHeadingRef} tabIndex={-1} className="text-2xl font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100 dark:text-gray-100 dark:text-slate-100 dark:text-slate-100 mb-6 focus:outline-none">Review Your Selection</h2>
            <div className="bg-blue-50 dark:bg-blue-950/20 dark:bg-blue-950/20 dark:bg-blue-950/20 dark:bg-blue-950/20 border-l-4 border-primary dark:border-primary/70 p-4 rounded mb-6">
            ...
            {tier?.features && tier.features.length > 0 && (
              <div className="mb-6">
                <h4 className="font-semibold text-gray-900 dark:text-gray-100 dark:text-gray-100 dark:text-gray-100 dark:text-slate-100 dark:text-slate-100 mb-3">Included Features</h4>
                <ul className="space-y-2">
                  {tier.features.slice(0, 5).map((feature, idx) => {
                    const featureText = typeof feature === 'string' ? feature : feature?.title || ''
                    return (
                      <li key={idx} className="flex items-start text-sm text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300">
                        <svg className="w-4 h-4 text-green-600 dark:text-green-400 dark:text-green-400 dark:text-green-400 mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        {currentStep === 'addons' && availableAddOns.length > 0 && (
          <div>
            <h2 ref={stepHeadingRef} tabIndex={-1} className="text-2xl font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100 dark:text-gray-100 dark:text-slate-100 dark:text-slate-100 mb-6 focus:outline-none">Customize Your Service</h2>
            ...
            {/* Price Summary */}
            <div ref={priceLiveRegionRef} aria-live="polite" className="bg-gray-50 dark:bg-slate-800/50 dark:bg-slate-800/50 p-4 rounded-lg mb-6">
              <div className="flex justify-between mb-2">
                <span className="text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300">Base Price:</span>
                <span className="font-semibold">{currentCurrency.symbol}{Math.round(convertedPrice).toLocaleString()}</span>
              </div>
              {addOnsTotal > 0 && (
                <div className="flex justify-between mb-2">
                  <span className="text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300">Add-Ons:</span>
        ...
        {currentStep === 'details' && (
          <div>
            <h2 ref={stepHeadingRef} tabIndex={-1} className="text-2xl font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100 dark:text-gray-100 dark:text-slate-100 dark:text-slate-100 mb-6 focus:outline-none">Your Details</h2>
            <div className="space-y-4">
              <div>
                <label htmlFor="name" className="block text-sm font-semibold text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300 dark:text-slate-300 mb-1">
                  Full Name *
                </label>
                <input
                  id="name"
              ...
              <div>
                <label htmlFor="email" className="block text-sm font-semibold text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300 dark:text-slate-300 mb-1">
                  Email Address *
                </label>
                <input
                  id="email"
              ...
              <div>
                <label htmlFor="phone" className="block text-sm font-semibold text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300 dark:text-slate-300 mb-1">
                  Phone Number *
                </label>
                <input
                  id="phone"
              ...
              <div>
                <label htmlFor="company" className="block text-sm font-semibold text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300 dark:text-slate-300 mb-1">
              ...
              <div>
                <label htmlFor="details" className="block text-sm font-semibold text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300 dark:text-slate-300 mb-1">
        ...
        {currentStep === 'payment' && (
          <div>
            <h2 ref={stepHeadingRef} tabIndex={-1} className="text-2xl font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100 dark:text-gray-100 dark:text-slate-100 dark:text-slate-100 mb-6 focus:outline-none">Order Summary & Payment</h2>
            
            {/* Order Summary */}
            <div className="bg-gray-50 dark:bg-slate-800/50 dark:bg-slate-800/50 p-4 rounded-lg mb-6">
              <h3 className="font-semibold text-gray-900 dark:text-gray-100 dark:text-gray-100 dark:text-gray-100 dark:text-slate-100 dark:text-slate-100 mb-4">Order Details</h3>
              <div className="space-y-2 mb-4">
                <div className="flex justify-between">
                  <span className="text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300">{serviceName}</span>
                  <span className="font-semibold">{currentCurrency.symbol}{Math.round(convertedPrice).toLocaleString()}</span>
                </div>
                {selectedAddOns.size > 0 && (
                  <div className="ml-4 space-y-1">
                    {Array.from(selectedAddOns).map(key => {
                      ...
                      return (
                        <div key={key} className="flex justify-between text-sm text-gray-600 dark:text-slate-400">
            ...
            <div className="bg-blue-50 dark:bg-blue-950/20 dark:bg-blue-950/20 dark:bg-blue-950/20 dark:bg-blue-950/20 p-4 rounded-lg mb-6 border border-blue-100 dark:border-blue-900/50">
              <p className="text-sm text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300 dark:text-slate-300 mb-2"><strong>Contact:</strong> {formData.name} ({formData.email})</p>
              {formData.company && <p className="text-sm text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300 dark:text-slate-300 mb-2"><strong>Company:</strong> {formData.company}</p>}
              <p className="text-sm text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300"><strong>Phone:</strong> {formData.phone}</p>
            </div>

            <p className="text-xs text-gray-600 dark:text-slate-400 dark:text-slate-400 mb-6">
        ...
        <div className="flex gap-4 mt-8">
          {currentStep !== 'review' && (
            <button
              onClick={handleBack}
              className="flex-1 min-h-[44px] border-2 border-gray-300 text-gray-700 dark:text-gray-300 dark:text-gray-300 dark:text-gray-300 dark:text-slate-300 dark:text-slate-300 py-3 px-6 rounded-xl font-semibold hover:border-gray-400 active:scale-95 transition-all"
            >
              Back
            </button>
          )}
