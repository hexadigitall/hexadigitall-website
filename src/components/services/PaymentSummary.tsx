...
export const PaymentSummary: React.FC<PaymentSummaryProps> = ({
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="p-6">
        <div className="mb-8">
          <h3 className="text-2xl font-bold text-gray-900 dark:text-slate-100 dark:text-slate-100 mb-2">Review & Payment</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div className="bg-gray-50 dark:bg-slate-800/50 dark:bg-slate-800/50 rounded-lg p-6">
              <h4 className="text-lg font-semibold text-gray-900 dark:text-slate-100 dark:text-slate-100 mb-4">Order Summary</h4>
              
              {/* Service Package */}
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h5 className="font-medium text-gray-900 dark:text-slate-100">{selectedPackage.name}</h5>
              ...
              {selectedAddOns.length > 0 && (
                <div className="border-t pt-4 mb-4">
                  <h6 className="font-medium text-gray-900 dark:text-slate-100 dark:text-slate-100 mb-2">Add-ons</h6>
                  {selectedAddOns.map((addOn) => (
                    <div key={addOn._key} className="flex justify-between items-center mb-2">
                      <div>
                        <span className="text-sm text-gray-900 dark:text-slate-100">{addOn.name}</span>
            ...
            <div className="bg-blue-50 dark:bg-blue-950/20 rounded-lg p-6">
              <h4 className="text-lg font-semibold text-gray-900 dark:text-slate-100 dark:text-slate-100 mb-4">What&apos;s Included</h4>
                ...
                {selectedPackage.features.map((feature, index) => {
                  ...
                  return (
                    <li key={index} className="flex items-start">
                      ...
                      <span className="text-gray-700 dark:text-slate-300 dark:text-slate-300 text-sm">{text}</span>
                    </li>
                  )
                })}
              </ul>
          ...
          <div className="space-y-6">
            {/* Client Information */}
            <div className="bg-white dark:bg-slate-900 dark:bg-slate-900 border rounded-lg p-6">
              <h4 className="text-lg font-semibold text-gray-900 dark:text-slate-100 dark:text-slate-100 mb-4">Client Information</h4>
              <div className="space-y-3">
                <div>
                  <span className="text-sm font-medium text-gray-600 dark:text-slate-400">Name:</span>
                  <p className="text-gray-900 dark:text-slate-100 dark:text-slate-100 mt-1">{clientInfo.firstName} {clientInfo.lastName}</p>
                </div>
                <div>
                  <span className="text-sm font-medium text-gray-600 dark:text-slate-400">Email:</span>
                  <p className="text-gray-900 dark:text-slate-100 dark:text-slate-100 mt-1">{clientInfo.email}</p>
                </div>
                {clientInfo.phone && (
                  <div>
                    <span className="text-sm font-medium text-gray-600 dark:text-slate-400">Phone:</span>
                    <p className="text-gray-900 dark:text-slate-100 dark:text-slate-100 mt-1">{clientInfo.phone}</p>
                  </div>
                )}
                {clientInfo.company && (
                  <div>
                    <span className="text-sm font-medium text-gray-600 dark:text-slate-400">Company:</span>
                    <p className="text-gray-900 dark:text-slate-100 dark:text-slate-100 mt-1">{clientInfo.company}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {projectDetails.notes && (
              <div className="bg-white dark:bg-slate-900 dark:bg-slate-900 border rounded-lg p-6">
                <h4 className="text-lg font-semibold text-gray-900 dark:text-slate-100 dark:text-slate-100 mb-4">Additional Notes</h4>
                <p className="text-gray-700 dark:text-slate-300 dark:text-slate-300 text-sm leading-relaxed">{projectDetails.notes}</p>
              </div>
            )}
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-8 mt-8 border-t">
          <button
            ...
            className="sm:w-auto px-6 py-3 border border-gray-300 text-gray-700 dark:text-slate-300 dark:text-slate-300 rounded-lg hover:bg-gray-50 dark:bg-slate-800/50 dark:bg-slate-800/50 transition-colors disabled:opacity-50"
          >
            Back to Details
          </button>
          <button
