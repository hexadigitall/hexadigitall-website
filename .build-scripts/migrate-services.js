...
const isMain = (() => {
    ...
    if (scriptPath.endsWith('migrate-services.ts') || scriptPath.endsWith('migrate-services.js'))
        return true;
    return false;
})();
if (isMain) {
...
export function formatPackageGroups(legacyPackages) {
    if (!Array.isArray(legacyPackages) || legacyPackages.length === 0)
        return [];
    // Narrow to expected shape
    const packages = legacyPackages.map((p) => (typeof p === 'object' && p !== null ? p : {}));
    // Simple heuristic: remove the first word from package names to derive a group name
    const firstName = (packages[0] && packages[0].name) || '';
    ...
    const tiers = packages.map((p) => {
        ...
        return ({
            ...
            price: (_b = p.price) !== null && _b !== void 0 ? _b : null,
            currency: (_c = p.currency) !== null && _c !== void 0 ? _c : 'USD',
            billing: (_d = p.billing) !== null && _d !== void 0 ? _d : 'one-time',
